...
describe("formatTime()", () => {
  ...
  it("creates the correct array for multiple objects with time formatted correctly", () => {
    let input = [
      {
        title: "Living in the shadow of a great man",
        topic: "mitch",
...
describe("formatData()", () => {
  it("if the array is empty it returns an empty object", () => {
    const actual = formatData([]);
    const expected = [];
    expect(actual).toEqual(expected);
  });
  it("returns object with key changed for a single object array", () => {
    const arrOfObj = [
      {
        body:
  ...
  it("doesnt mutate the array of objects passed in", () => {
    const arrOfObj = [
      {
        body:
          "Oh, I've got compassion running out of my nose, pal! I'm the Sultan of Sentiment!",
